<template>
  <div :class="paddingBtm" element-loading-text="拼命加载中">
      ...
      <div class="bd-top" style="border:none">
        <div class="md clearfix">
          ...
          <div class="md-right" >      
            ...
            <el-menu  @select="changePerson" :default-active="caseCode"  active-text-color="#409EFF">
               <el-menu-item  v-for="item in dropdownData" :key="item.id" :index="item.caseCode">
                  <span >{{item.name}} </span>
               </el-menu-item>
            </el-menu>
        ...
        <div class="case-detail">
          <el-row>
            ...
            <div class="borrower-tags">
              <p v-if="labels.self">
                <span>本人：</span>
                <el-tag size="mini"  v-for="btn in labels.self" round :key="btn.id">{{btn}}</el-tag>
              </p>  
              <p v-if="labels.direct">
                <span>直系：</span> 
                <el-tag size="mini" v-for="btn in labels.direct" round :key="btn.id">{{btn}}</el-tag>
              </p>
              <p v-if="labels.others">
                <span>其他：</span>
                <el-tag size="mini" v-for="btn in labels.others" round :key="btn.id">{{btn}}</el-tag>
              </p>
            </div>
          </el-row>
        </div>
        <information :debtInfoBeans="debtInfoBeans" :caseInfoBeans="caseInfoBeans" :userInfoBean="userInfoBean" :moreInfo="moreInfo" :stopList="stopList"
          @toMoreInfo="toMoreInfo" @callPhone="callPhone" @smsSend="smsSend" @qqSend="qqSend" @wechatSend="wechatSend" @chooseConcat="chooseConcat"
          ref="information" @changeCustomer="changeCustomer"  @addUser="addUser" :userInfoList="userInfoList" :qqUniqId="qqUniqId" :wechatUniqId="wechatUniqId" :phoneUniqId="phoneUniqId" ></information>
          <div style="text-align:center;">
            <el-button type="text" @click="toMoreInfo">点击查看 更多相关信息</el-button>
          </div>
    ...
    <div class="content-body">
      ...
      <div class="bd-main">
        ...
        <!-- 留案 -->  
        <caseLeave v-if="componentIndex==8" :caseLeaveTb="caseLeaveTb" :caseLeaveParam="params" @getcaseLeaveList='getcaseLeaveList'
          :caseDisable="caseDisable"></caseLeave>
        ...
        <div class="infoLeft">
          <el-form inline class="condition-form" label-position="right" :model="callData" ref="phoneStatus">
            ...
            <el-form-item label="呼叫电话：" prop='callPhone' :error="stopError">
              <el-input v-model="callData.callPhone" :maxlength="20" placeholder="请输入拨打号码" size="mini" :disabled="repaireDisBtn" @change="inputPhone" >
              </el-input>
          ...
          <el-form inline class="condition-form" label-position="right" label-width="80px" :model="callData" ref="phoneLabel">
            <el-form-item label="电话标注：" style="width:100%" prop="phoneLabel" :rules="[{required:true,message:'电话标注必选项',trigger:'change'}]">
              <el-radio-group v-model="callData.phoneLabel" size="mini">
                ...
                <el-radio-button label="忙音"></el-radio-button> 
        ...
        <div class="infoRight">
          <el-form inline class="condition-form" label-position="right"   ref="collectionRemark" :model="callData">
            <el-form-item style="width:100%" prop="collectionRemark"   :rules="[{required:true,message: '催记详情为必填项', trigger:'blur'}]">
              <el-input type="textarea" placeholder="添加更多催记详情" v-model="callData.collectionRemark" :rows="12" ></el-input>
            </el-form-item>
          </el-form>
        </div>
        <div>
          <div class="promiseInfo" v-if="promiseInfoShow">
            <el-form inline class="condition-form" label-position="right" label-width="120" :model="callData" :rules="rules" ref="promise">
              <el-form-item label="还款承诺：" label-width="80px">
              </el-form-item>
              ...
              <el-form-item label="承诺还款金额：" prop="promiseAmount" ref="promiseAmount">
          ...
          <div class="addArea">
            <el-form inline class="condition-form concatInfo" label-position="right" label-width="120" :model="addPersonInfo" ref="addPersonInfo"
              ...
              <el-form-item label="手机号：" prop="contactTel" :rules="[{required:true,message:'必填项',trigger:'blur'}]">
    ...
    <addContacts :caseCode="caseCode" :addContactsVisible="addContactsVisible" @closeDialog="closeDialog" ref="addContacts" @refreshUserList="refreshUserList"></addContacts>
  </div>
</template>
...
<script>
  ...
  //  留案
  import caseLeave from './case-leave/case_leave'
  // 投诉
  import complain from './complain/add_complain'
  // 备忘
  import note from './note/add_note'
  import addContacts from './addContacts_dialog'
  import {
    mapGetters,
    mapActions
  } from 'vuex'
  export default {
    ...
    created() {
      ...
      window.addEventListener('message', function (e) {
        if (e.data && (typeof e.data == 'string')) {
          ...
          if (arr != null && arrCaller ) {
            let incoming_call = arr[1];
            let callerPhone = arrCaller[1];
            let calleePhone = arrCallee[1];
            // 拨打分机号不同，不是主叫，停止监听
            if(callerPhone != this.callUserName){return false;}
            this.callId = incoming_call;
            this.callData.callId = this.callId;
            // 失联修复主叫，
            if(this.phoneType == 1) {
              // 定义失联修复转换后的电话，
               var encryptCalleePhone,calleePhone = calleePhone +'',len = calleePhone.length;
               // 4位不变
               if(len<4) {
                 encryptCalleePhone = calleePhone
               } else{
                 //去最后四位
                 let split = calleePhone.substr(calleePhone.length-4,4);
                 // 大于8位，倒数5-8位变为*
                 if(len>=8){
                   encryptCalleePhone = calleePhone.substring(0,calleePhone.length-8)+'****'+split;
                 } else {
                   // 4-7 最后四位为*
                   encryptCalleePhone = calleePhone.substring(0,calleePhone.length-4)+'****';
                 }
               }
               if(this.callData.callPhone != encryptCalleePhone) return false;
            } else {
              if(this.callData.callPhone != calleePhone) return false;
            }
    ...
    methods: {
        ...
        this.$axios
          ...
          .then(res => {
            if (res.data.code == 0) {
              ...
              this.userInfoBean = data.userInfoBeanList[0];
              ...
              this.userInfoList = data.userInfoBeanList;
              ...
              this.callData.callPhone = this.userInfoBean.listPhone[0];
              ...
              this.disabledBtn = this.isStop = data.stopList ? Array.includes(this.stopList,this.callData.callPhone) ? true:false:false;
              this.getPhoneAddress(this.callData.callPhone);
              this.callData.phoneStatus = '有效';
              ...
              this.$refs.addContacts.refreshCaseCode(this.caseCode);
              this.repaireDisBtn = false;
              this.callData.addContact = [];
              this.phoneType = 0;
              this.callData.strongId = '';
              this.callData.source = '';
              this.callData.isRelevancy = 0;
              this.repaireDisBtn = false;
              this.infoShow = true;
            } else {
              this.$util.failCallback(res.data, this);
      ...
      submit(item) {
        ...
        if(this.callData.addContact.length != 0) {
          this.$refs.addPersonInfo.forEach( (item,index) => {
              this.$refs.addPersonInfo[index].validate(valid => {
              if (valid) {
                error = false;
              } else {
                error = true;
      ...
      changeCustomer(uniqueId) {
        this.userInfoList.forEach( (item)=> {
          if(item.uniqueId == uniqueId) {
            ...
            if(this.stopList.includes(this.userInfoBean.listPhone[0])) {
              this.isStop = true;
              this.disabledBtn = true;
            } else {
              this.isStop = false;
      ...
      onCall() {
        ...
        Array.includes(this.stopList,this.callData.callPhone)?this.isStop = true:this.isStop = false
        this.getEncodePhone();
      },
      preventAction() {
      ...
      // 获取留案列表
       getcaseLeaveList() {
        this.$axios
          .post("/api/assignee/collectionManage/queryCaseLeaveApply", {
            caseCode: this.caseCode,
            caseId: this.caseId,
            caseManageId: this.caseManageId
          })
          .then(res => {
            if (res.data.code == 0) {
              this.caseLeaveTb = res.data.data;
            } else {
              this.$util.failCallback(res.data, this);
            }
          })
      ...
      // 电话列表-选择联系人
      chooseConcat(item,error) {
        this.callData = this.assignData;
        this.callData.caseId = this.caseId;
        this.callData.caseCode = this.caseCode;
        this.callData.caseManageId = this.caseManageId;
        this.callData.callName = this.userInfoBean.borrowerName;
        this.callData.callPhone = item;
        this.callData.contactRelation = this.userInfoBean.contactRelation;
        this.isStop = error;
        this.disabledBtn = error;
        this.phoneType = 0;
        this.callData.strongId = '';
        this.callData.source = '';
        this.callData.isRelevancy = 0;
      ...
      inputPhone(phone) {
        ...
        if (this.fixPhone == phone && phone) {
          ...
          for (var item of this.userInfoBean.listPhone) {
            if (item == phone) {
              this.callData.callName = this.userInfoBean.borrowerName;
              break;
            } else {
