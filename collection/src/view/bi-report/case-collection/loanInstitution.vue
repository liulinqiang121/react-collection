  ...
  <div class="content-body">
    ...
    <div class="bd-main">
      <el-form :inline="true" class="condition-form" size="mini" >
        <div>
          <el-form-item class="el-col fixed-width">
          ...
          <el-form-item label="账龄" prop="receivableAge" class="el-col fixed-width">
          ...
          <el-form-item label="手别" prop="batchTimes" class="el-col fixed-width">
            <el-select placeholder="请选择手别" v-model="conditionForm.batchTimes"  clearable size="mini">
...
<script>
  ...
  export default {
    ...
    methods: {
      getbatchTimes() {
        ...
        this.$axios.post('/api/assignee/caseCollection/loanInstitution/getAllBatchTimes', {}).then((res) => {
          if (res.data.code == 0) {
            ...
            this.batchTimes.unshift({code: '',name: 'All'})
          } else {
      ...
      getreceivableAge() {
        ...
        this.$axios.post('/api/assignee/caseCollection/loanInstitution/getAllReceivableAge', {}).then((res) => {
          if (res.data.code == 0) {
            ...
            this.receivableAge.unshift({code: '',name: 'All'})
      ...
      drawChart(chartObj, data, title,legend) {
        var option = {
          title: {
            text: title,
            left: 5,
      ...
      getrepayMoneyChartData() {
        this.$axios.post('/api/assignee/caseCollection/loanInstitution/getMoneyData', this.conditionForm).then((res) => {
          if (res.data.code == 0) {
            ...
            this.drawChart(this.repayMoneyChart, whichChart, '案件回收金额分析',['委案金额', '回收金额'])
          } else {
            this.$util.failCallback(res.data, this)
          }
        }).catch((err) => {
      ...
      getrepayCountChartData() {
        this.$axios.post('/api/assignee/caseCollection/loanInstitution/getCountData',this.$util.encodePostBody(this.conditionForm)).then((res) => {
          if (res.data.code == 0) {
            let whichChart = {
              xAxisData: [],
              seriesData: [
            ...
            this.drawChart(this.repayCountChart, whichChart, '案件回收数量分析',['委案数量', '回收数量'])
          } else {
            this.$util.failCallback(res.data, this)
          }
        }).catch((err) => {
