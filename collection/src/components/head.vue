  ...
  export default {
    ...
    methods: {
      ...
      redirect(row) {
        ...
        if(this.isPrecall) {this.PreCallMethod(row.caseId)};  
      ...
      // 预测式外呼数据回调
      PreCallMethod(caseId){
        this.$axios
          .post("/api/assignee/call/updatePreCallDetailCaseInfo", {
            caseId: caseId,
          }).then(() => {
          ...
          if (arrCallee && arr && arrCaller) {
            ...
            if (backData.status_notify) {
              ...
              switch (backData.status) {
                ...
                case 'Error':
                  {
                    if (backData.event && backData.event == 'loginReturn' && backData.code !='1006') {
                      if (backData.msg != 'Error') {
                        this.$message.error('呼叫系统' + backData.msg)
                      }
                    } else {
                       if (backData.code == "1006") {
                       this.$confirm('呼叫系统已在其他地方登录，是否强制对方退出','提示',{
                       ...
                       .then(() => {
                          this.$axios
                            .post("/api/assignee/call/forceLogout", {})
                            .then(res => {
                              if(res.data.code == 0){
                                  this.getLoginInfo()
                              } else {
