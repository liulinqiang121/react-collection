<template>
  <div>
    <div class="md">
      ...
      <div class="md-right">
        <el-button size="mini" type="primary" @click="openDialog" v-if="visibleBtn">添加减免凭证</el-button>
      </div>
    </div>
    ...
    <el-dialog title="添加减免凭证" :close-on-click-modal="false" :show-close="true" :visible.sync="dialogVisible" :center="true" @close="closeDialog(dialogForm)">
      <el-form ref='dialogForm' :rules="rules" label-width="100px" :model="dialogForm" :inline="true" size="mini">
        ...
        <el-form-item label="减免要求" v-if="postData.depreciationId">
          {{depreciationNeed}}
        </el-form-item>
        <el-form-item>
    ...
    <!-- 查看拒绝原因 -->
     <el-dialog title="拒绝详情" :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="dialogClose">
          <el-form :model="refuseForm" label-width="100px" ref="refuseForm">
        ...
        this.$axios
          ...
          .then(res => {
            if (res.data.code == 0) {
              ...
              if(this.applyderateTb.length) {
                 this.applyderateTb[0].isFile?this.visibleBtn = false:this.visibleBtn = true;
                 console.log(this.applyderateTb[0])
               } else {
