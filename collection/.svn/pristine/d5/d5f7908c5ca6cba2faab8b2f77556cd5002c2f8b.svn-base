    ...
    <el-dialog title="添加用户" :close-on-click-modal="false" :show-close="true" :visible.sync="userDialog" :center="true" @close="closeDialog"
      ...
      <el-form ref="form" :model="form" label-width="120px" size="small" :rules="rules" class="dialog-main">
        ...
        <el-form-item label="职位" prop="position">
          <el-select v-model="form.position" placeholder="请选择职位" clearable @change="positionChange" value-key="id">
            <el-option v-for="option in options" :label="option.positionName" :value="option" :key="option.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="职能"  v-if="form.position&&form.position.positionType == 'ADMINISTRATOR'">
          <el-select v-model="form.caseAuthority.loanInstitutions" multiple clearable placeholder="请选择">
            <el-option v-for="item in caseAuthorityOptions.loanInstitutions" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm(form) {
        this.$refs.form.validate((valid) => {
          ...
          // this.form.postionId = this.form.postionId
          if (this.created) {
            this.$axios.post('/api/assignee/user/createUser', 
                this.form
              )
              .then(res => {
                if (res.data.code == 0) {
            ...
            this.$axios.post('/api/assignee/user/updateUser', this.form)
              .then(res => {
                if (res.data.code == 0) {
      ...
      // 平台三期需求——文员只能查看及分发自己负责的银行的案件
      positionChange(val) {
        this.form.postionId = val.id;
        this.form.caseAuthority = {
          loanInstitutions:[]
        };
        // if (val.positionType == "ADMINISTRATOR") {
        //   this.isAdmin = true;
        // }else{
        //   this.isAdmin = false;
        // }
    ...
    created() {
      ...
      // 获取职能
      this.$axios.get('/api/assignee/user/caseAuthority')
        .then(res => {
          if (res.data.code == 0) {
            this.caseAuthorityOptions = res.data.data;
          } else {
            this.$util.failCallback(res.data, this);
          }
        })
