  ...
  <div class="content-body">
    ...
    <div class="bd-main">
      <el-form ref="conditionForm" label-width="500px" label-position="top">
        ...
        <el-form-item label="案件查看权限" prop="">
          <span class="set-label">查看过期案件</span>
          <el-switch v-model="cover.overdueCase.hasOpen" active-color="#13ce66" inactive-color="#ff4949" :active-value="true" :inactive-value="false"
            @change="switchChange(cover.overdueCase)">
          </el-switch>
        </el-form-item>
        <el-form-item label="每天短信发送次数" prop="">
          <span class="set-label">
          </span>
          <el-select v-model="form.assSmsTimes" placeholder="请选择" size="small">
            <el-option
              v-for="item in dropdownData.assSmsTimes"
              :key="item.code"
              :label="item.name"
              :value="item.code">
            </el-option>
        ...
        this.$axios
          ...
          .then(res => {
            if (res.data.code == 0) {
              ...
              this.form.assSmsTimes = res.data.data.assSmsTimes.code;
            } else {
              this.$util.failCallback(res.data, this);
            }
          })
