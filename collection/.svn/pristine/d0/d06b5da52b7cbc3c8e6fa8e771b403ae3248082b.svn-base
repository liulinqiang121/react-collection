<template>
  <div class="sms">
    <el-dialog :title="'( '+this.phoneInfo.relation+' )'+this.phoneInfo.phone" :visible.sync="smsVisible" :close-on-click-modal="false"
      @open="getSmsInfo" :before-close="handleClose">
      <div class="latestSms" v-if="latestSms">
        <p>{{latestSms}}</p>
      </div>
      <el-form label-width="80px" label-position="left">
        <el-form-item label="短信模板">
          <el-select v-model="templateName" value-key="templateName" placeholder="请选择" size="mini" class="selection" @change="templateChange" clearable>
...
<script>
  export default {
    ...
    methods: {
      getSmsInfo() {
        this.$axios
          .post("/api/assignee/sms/getSmsInfo", {
