  ...
  <div>
    ...
    <div class="content-body">
      ...
      <div class="bd-main">
        ...
        <el-table ref="multipleTable" height="460" :data="tb.data" tooltip-effect="dark" v-loading="loading" element-loading-text="拼命加载中"
          element-loading-spinner="el-icon-loading" @selection-change="handleSelectionChange" show-overflow-tooltip>
          ...
          <el-table-column width="120" label="操作" align="center" fixed="right" v-if="operationBtns[0].isShow && operationBtns[1].isShow">
            <template slot-scope="scope">
              <el-button size="mini" v-on:click.stop.prevent="playRecord(scope)" :disabled="scope.row.calledTime == 0 || scope.row.calledTime == null" type="text" :class="scope.row.icon" v-if="operationBtns[0].isShow">
              </el-button>
              <el-button size="mini" v-on:click.stop.prevent="downloadRecord(scope)" :disabled="scope.row.calledTime == 0 || scope.row.calledTime == null" type="text" class="el-icon-download" v-if="operationBtns[1].isShow">
              </el-button>
              <el-button size="mini" v-on:click.stop.prevent="checkRecord(scope)" :disabled="scope.row.replyStatus== '1' || scope.row.calledTime == 0 || scope.row.calledTime == null" v-if="operationBtns[2].isShow"
                type="text" class="el-icon-edit-outline" >
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-dialog :close-on-click-modal="false" :show-close="true" :visible.sync="dialogVisible" :center="true" @close="resetDialog"
        ...
        <el-form ref='dialogForm' label-width="140px" :model="dialogForm" class="dialog-main" size="mini">
          <el-form-item label="核实情况：" prop="checkDetail">
            <el-input type="textarea" v-model="dialogForm.checkDetail" placeholder="请输入核实情况" size="mini" ></el-input>
          </el-form-item>
          <el-form-item label="安抚记录：" prop="appeaseRecord">
            <el-input type="textarea" v-model="dialogForm.appeaseRecord" placeholder="请输入安抚记录" size="mini" ></el-input>
          </el-form-item>
        </el-form>
...
<script>
  ...
  export default {
        ...
        operationBtns: [{
          ...
          },
          {
            ...
            dialog: {
              ...
              title: '导出列表',
              submitUrl: '/api/assignee/complaintPrewarning/export',
              downloadTemplateUrl: '/api/assignee/complaintPrewarning/downloadTemplate',
              beforeOpenDialogUrl: '/api/assignee/complaintPrewarning/hasTemplate',
            },
            isShow: true,
          },
    ...
    methods: {
      // 获取下拉列表
      getDownList() {
        this.$axios.post("/api/assignee/complaintPrewarning/getSearchInfo", {}).then(
            res => {
              if (res.data.code == 0) {
                ...
                this.dropdownData.talkTime.unshift({
                  name: 'All',
                  code: ''
                });
                ...
                this.dropdownData.relation.unshift({
                  name: 'All',
                  code: ''
                });
              } else {
      ...
      remoteMethod(query) {
        if (query !== '') {
          this.$axios
            .post("/api/assignee/complaintPrewarning/getStaff", {
              collectionId: query
            })
            .then(res => {
              if (res.data.code == 0) {
      ...
      playRecord(scope) {
        ...
        if (scope.row.icon == 'el-icon-service') {
          ...
          this.$axios({ // 用axios发送post请求
              method: 'post',
              url: '/api/assignee/complaintPrewarning/download', // 请求地址
              data: {
      ...
      exportRecord() {
        ...
        this.$axios({ // 用axios发送post请求
            method: 'post',
            url: '/api/assignee/complaintPrewarning/batchExport', // 请求地址
            data: {
              ids: this.selected
            }, // 参数
            responseType: 'blob' // 表明服务器返回的数据类型
      ...
      getList(data) {
        ...
        this.loading = true;
        this.$axios
          .post("/api/assignee/complaintPrewarning/search", queryParams)
          .then(res => {
            this.loading = false;
      ...
      // 添加质检
      checkRecord(scope) {
        this.dialogForm.id = scope.row.id;
      ...
      submitDialog() {
        this.$axios
          .post("/api/assignee/complaintPrewarning/reply", this.dialogForm)
          .then(res => {
            if (res.data.code == 0) {
