    ...
    <div class="content-body">
      ...
      <div class="bd-main">
        <el-form ref="conditionForm" :model="conditionForm" :label-width="this.$util.LABEL_WIDTH" label-position="right" class="condition-form"
          ...
          <div class="el-col fixed-width">
            <el-form-item label="委案时间" prop="commitDate">
              <el-date-picker v-model="conditionForm.commitDate" value-format="yyyy-MM-dd" type="daterange" @change="consignerDateChange"
          ...
          <div class="el-col fixed-width">
            <el-form-item label="退案时间" prop="limitDate">
              <el-date-picker v-model="conditionForm.limitDate" value-format="yyyy-MM-dd" type="daterange" @change="limitDateChange" start-placeholder="开始日期"
                end-placeholder="结束日期" clearable>
              </el-date-picker>
            </el-form-item>
          </div>
          <div class="el-col fixed-width">
            <el-form-item label="批次分配状态" prop="batchAllotStatus">
       ...
       <!-- <el-table ref="multipleTable" height="650" :data="tb.data" tooltip-effect="dark" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
        ...
        <my-table ref="multipleTable" :tb="tb"  :loading="loading" @choose="handleSelectionChange" @sortChange="sortChange" @goDetail="goDetail"></my-table>
  ...
  export default {
    ...
    methods: {
      ...
      goDetail(scope, caseListType) {
        ...
        // }))
        var type, batchCode = scope.row.batchCode,batchName = scope.row.batchName;
        ...
        var url = (window.location.origin ? window.location.origin : '') + '/#/assignee_batch_detail?type=' + type + '&batchCode=' + enBatchCode +'&batchName='+enBatchName;
        window.open(url);
      },
      openDialog(btn) {
      ...
      getdropdownData(cb) {
        ...
        this.$axios.post('/api/assignee/caseManage/getBatchManageSearchInfo', {}).then((res) => {
          if (res.data.code == 0) {
            ...
            this.dropdownData.batchAllotStatus.unshift({
              name: 'All',
              code: ''
            })
